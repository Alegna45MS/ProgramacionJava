...
public class Wordle {
	...
	public static boolean comprobarPalabra(String palabra) {
		...
		if (palabra.contains("aa") || palabra.contains("ee") || palabra.contains("ii") || palabra.contains("oo")
				|| palabra.contains("uu") || palabra.contains("AA") || palabra.contains("EE") || palabra.contains("II")
				|| palabra.contains("OO") || palabra.contains("UU")) {
			return false;

			// Si la posicion 4 de la palabra es una 'q','w' o 'x',no vale
		}
		if (palabra.toLowerCase().charAt(4) == 'q' || palabra.toLowerCase().charAt(4) == 'w'
				|| palabra.toLowerCase().charAt(4) == 'x') {
			return false;
		}
		return true;
	}
